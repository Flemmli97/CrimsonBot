...
async def _purge(channel: discord.TextChannel | discord.channel.VocalGuildChannel | discord.Thread,
...
class PurgeCommand(commands.Cog):
    ...
    async def _purge(self, interaction: discord.Interaction,
        ...
        msg = await _purge(interaction.channel, amount, check, after, oldest_first, exclude=(await interaction.original_response()).id)
        msgs = len(msg)
        embed = discord.Embed(
            description=f"Purged {msgs} messages" if msgs != 1 else "Purged 1 message",
            color=EMBED_COLOR
        )
