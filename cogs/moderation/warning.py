...
class Warning(commands.Cog):
    ...
    async def warn(self, interaction: discord.Interaction, user: discord.Member, reason: str, severity: Optional[int]):
    ...
    async def get_warns(self, interaction: discord.Interaction, user: discord.Member):
        ...
        all_warnings = {by: sorted(warning, key=lambda w: w.created_at) for by, warning in all_warnings.items()}
    ...
    async def clear_warnings(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if len(result) == 0:
            await interaction.response.send_message(f"{user.mention} has no warnings.")
            return
        await self.warnings.remove(interaction.guild.id, user=user.id)
